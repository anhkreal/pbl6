...
import { fetchEmployees, Employee } from '../../api/employees';
...
export default function AdminAnalysis() {
  ...
  useEffect(() => { (async () => { const list = await fetchEmployees(); setUsers(list); if (list.length) setUserId(list[0].id); })(); }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        if (mode === 'day') {
          ...
          const items = await fetchKPI('day', day, userId, 1, 0);
          if (!ignore && items?.length) setKpi(items[0].totalScore);
          const logs = await fetchEmotionLogs({ user_id: userId, start_ts: start.toISOString(), end_ts: end.toISOString(), limit: 500, offset: 0 });
          if (!ignore) setNegDist(toNegDist(logs.logs));
          const chk = await fetchCheckLogs({ user_id: userId, date_from: day, date_to: day, limit: 100, offset: 0 });
          ...
          const { from, to } = monthBoundsGmt7(month);
          const items = await fetchKPI('month', month, userId, 1, 0);
          if (!ignore && items?.length) setKpi(items[0].totalScore);
          const logs = await fetchEmotionLogs({ user_id: userId, start_ts: from.toISOString(), end_ts: to.toISOString(), limit: 2000, offset: 0 });
          ...
          const chk = await fetchCheckLogs({ user_id: userId, date_from: fmtYmd(from), date_to: fmtYmd(to), limit: 1000, offset: 0 });
    ...
    return () => { ignore = true; };
  }, [userId, mode, day, month, refresh]);

  const conclusion = useMemo(() => buildConclusion({ kpi, negDist, hoursSeries }), [kpi, negDist, hoursSeries]);

  return (
    <AdminLayout>
      <h1 style={{ marginBottom: 16 }}>Phân tích theo nhân viên</h1>
      <div className="card" style={{ marginBottom: 12 }}>
        <div className="card-body" style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          <select value={userId ?? ''} onChange={e => setUserId(Number(e.target.value))}>
            {users.map(u => <option key={u.id} value={u.id}>{u.fullName || u.username || ('ID '+u.id)}</option>)}
          </select>
